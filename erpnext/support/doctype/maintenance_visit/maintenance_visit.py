...
class MaintenanceVisit(TransactionBase):
		...
		for d in self.get('maintenance_visit_details'):
			if d.prevdoc_docname and d.prevdoc_doctype == 'Customer Issue' :
				if flag==1:
					...
					nm = frappe.db.sql("select t1.name, t1.mntc_date, t2.service_person, t2.work_done from tabMaintenance_Visit t1, tabMaintenance_Visit_Purpose t2 where t2.parent = t1.name and t1.completion_status = 'Partially Completed' and t2.prevdoc_docname = %s and t1.name!=%s and t1.docstatus = 1 order by t1.name desc limit 1", (d.prevdoc_docname, self.name))
				...
				frappe.db.sql("update tabCustomer_Issue set resolution_date=%s, resolved_by=%s, resolution_details=%s, status=%s where name =%s",(mntc_date,service_person,work_done,status,d.prevdoc_docname))
	...
	def check_if_last_visit(self):
		...
		if check_for_docname:
			check = frappe.db.sql("select t1.name from tabMaintenance_Visit t1, tabMaintenance_Visit_Purpose t2 where t2.parent = t1.name and t1.name!=%s and t2.prevdoc_docname=%s and t1.docstatus = 1 and (t1.mntc_date > %s or (t1.mntc_date = %s and t1.mntc_time > %s))", (self.name, check_for_docname, self.mntc_date, self.mntc_date, self.mntc_time))
